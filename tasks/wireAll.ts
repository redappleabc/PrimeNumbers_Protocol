...
module.exports = async function (taskArgs, hre) {
	...
	let transactionBynetwork = await Promise.all(
		localNetworks.map(async (localNetwork) => {
			...
			localContractName =
				localContractName === undefined ? WIRE_UP_CONFIG['contractName'][env] : localContractName;
			...
			transactions.push(
				...(await setUseCustomAdapterParams(
					...
					localNetwork,
					localContractName,
					WIRE_UP_CONFIG[localNetwork].useCustomAdapterParams
				))
			);
			...
			await Promise.all(
				remoteNetworks.map(async (remoteNetwork) => {
					...
					remoteContractName =
						remoteContractName === undefined ? WIRE_UP_CONFIG['contractName'][env] : remoteContractName;
					...
					transactions.push(
						...(await setMinDstGas(
							...
							localNetwork,
							localContractName,
							WIRE_UP_CONFIG[localNetwork]['remoteNetworkConfigs'][remoteNetwork].minDstGasConfig,
							CHAIN_ID[remoteNetwork]
						))
					);
				})
			);
	...
	if (taskArgs.n) {
		await crossChainHelper.promptToProceed('Would you like to Submit to gnosis?', true);
		await Promise.all(
		...
		await crossChainHelper.promptToProceed('Would you like to run these transactions?', true);
		await Promise.all(
			transactionBynetwork.map(async ({network, transactions}) => {
				const transactionToCommit = transactions.filter((transaction) => transaction.needChange);
				...
				for (let transaction of transactionToCommit) {
					...
					try {
						...
						print[
							network
						].past = `${transaction.contractName}.${transaction.methodName} (${tx.transactionHash})`;
						successTx++;
...
async function setUseCustomAdapterParams(
	hre: any,
	localNetwork: string,
...
async function setTrustedRemote(
	...
	const desiredTrustedRemote = ethers.utils.solidityPack(
		['bytes'],
...
async function setInboundProofType(
	hre: any,
	localNetwork: string,
	localContractName: string,
	remoteChainId: number,
	inBoundProofType: number,
	lockDown: boolean = false
): Promise<Transaction[]> {
	const localContract = await crossChainHelper.getContract(hre, localNetwork, localContractName);
	const VERSION = 2;
	const CONFIG_TYPE_INBOUND_PROOF_LIBRARY_VERSION = 1;
	const cur = await localContract.getConfig(
		VERSION,
		remoteChainId,
		ethers.constants.AddressZero,
		CONFIG_TYPE_INBOUND_PROOF_LIBRARY_VERSION
	);
	let config = ethers.utils.defaultAbiCoder.encode(['uint16'], [inBoundProofType]);
...
async function setInboundBlockConfirmations(
	hre: any,
	localNetwork: string,
	localContractName: string,
	remoteChainId: number,
	inBoundConfirmations: number,
	lockDown: boolean = false
): Promise<Transaction[]> {
	const localContract = await crossChainHelper.getContract(hre, localNetwork, localContractName);
	const VERSION = 2;
	...
	let config = ethers.utils.defaultAbiCoder.encode(['uint16'], [inBoundConfirmations]);
	const needChange = lockDown ? lockDown : cur !== config;
...
async function setRelayer(
	...
	let config = ethers.utils.defaultAbiCoder.encode(['address'], [relayerAddress]);
	const needChange = lockDown ? lockDown : cur !== config;
...
async function setOutboundProofType(
	hre: any,
	localNetwork: string,
	localContractName: string,
	remoteChainId: number,
...
async function setOutboundBlockConfirmations(
	hre: any,
	localNetwork: string,
	localContractName: string,
	remoteChainId: number,
	...
	let config = ethers.utils.defaultAbiCoder.encode(['uint16'], [outBoundConfirmations]);
	const needChange = lockDown ? lockDown : cur !== config;
...
async function setOracle(
	...
	let config = ethers.utils.defaultAbiCoder.encode(['address'], [oracleAddress]);
	const needChange = lockDown ? lockDown : cur !== config;
...
async function lockToSendAndRecvVersion(
	hre: any,
	localNetwork: string,
	localContractName: string,
	config: any,
